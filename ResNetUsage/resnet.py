...
class ResNet(object):
    ...
    def resnet_layers_proto(self, batch_size, phase='TRAIN', stages=(3, 4, 6, 3)):
        ...
        for num in range(len(stages)):  # num = 0, 1, 2, 3
            for i in range(stages[num]):
                if i == 0:
        ...
        exec(str('n.pool5 = L.Pooling((bottom), pool=P.Pooling.AVE, global_pooling=True)').replace('(bottom)', 'n.res5b%s' %  str(stages[3] - 1)))
